...
@if($mode === 'map')
    <div
        class="{{ $classes['map'] }} bg-opacity-60 hover:bg-opacity-80 backdrop-blur-sm transition-all duration-500 flex flex-col justify-center items-center h-full min-h-[200px] border-2 border-white/30 rounded-lg group relative overflow-hidden">
        ...
        <div class="relative z-10 text-center text-white drop-shadow-md">
            ...
            <span
                class="bg-black/30 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border border-white/20">
                {{ $status }}
            </span>
        </div>
...
@else
    <div
        class="rounded-xl shadow-md border-l-4 p-5 {{ $classes['card'] }} transition transform hover:scale-105 duration-300 bg-white">
        ...
        <div class="mt-4 w-full bg-white bg-opacity-40 rounded-full h-2">
            <div class="h-2 rounded-full {{ str_replace('text-', 'bg-', $classes['subtext']) }}"
